...
from raretrap.backends.model_assets import load_model_assets
...
def build_obj(
    *,
    ...
    print(
        f"[init] requested_device_map={assets.requested_device_map or 'single-device'} "
        f"| resolved_device_map={assets.resolved_device_map or 'single-device'}"
    )
    print(f"[init] generator_backend={assets.generator_backend}")
    ...
    if assets.generator_backend == "vllm":
        from raretrap.backends.vllm_generator import VLLMTextGenerator, build_vllm_engine

        engine = build_vllm_engine(
            model_name=str(tested_llm_model_name),
            revision=assets.tested_model_revision or tested_model_revision,
    ...
    print(
        ...
        f"{getattr(generator, 'cache_implementation', None) or assets.generator_backend + '-default'}"
    )
    print(
        f"[init] generation_chunk_size="
        f"{getattr(generator, 'generation_chunk_size', None) or 'one-shot'}"
    )
    ...
    return ThinkTrapObjective(
        ...
        run_metadata={
            ...
            "model_device_map_requested": assets.requested_device_map,
            "model_device_map_resolved": assets.resolved_device_map,
            "generator_backend": assets.generator_backend,
            "vllm_tensor_parallel_size": (
                int(os.environ.get("THINKTRAP_VLLM_TENSOR_PARALLEL_SIZE", "1"))
                if assets.generator_backend == "vllm"
                else None
            ),
            "vllm_pipeline_parallel_size": (
                int(os.environ.get("THINKTRAP_VLLM_PIPELINE_PARALLEL_SIZE", "1"))
                if assets.generator_backend == "vllm"
                else None
            ),
            "vllm_quantization": (
                os.environ.get("THINKTRAP_VLLM_QUANTIZATION", "modelopt_fp4")
                if assets.generator_backend == "vllm"
                else None
            ),
            "vllm_language_model_only": (
                os.environ.get("THINKTRAP_VLLM_LANGUAGE_MODEL_ONLY", "0") == "1"
                if assets.generator_backend == "vllm"
                else None
            ),
            "vllm_enable_prefix_caching": (
                os.environ.get("THINKTRAP_VLLM_ENABLE_PREFIX_CACHING", "1") == "1"
                if assets.generator_backend == "vllm"
                else None
            ),
            "vllm_max_num_batched_tokens": (
                int(os.environ.get("THINKTRAP_VLLM_MAX_NUM_BATCHED_TOKENS", "32768"))
                if assets.generator_backend == "vllm"
                else None
            ),
